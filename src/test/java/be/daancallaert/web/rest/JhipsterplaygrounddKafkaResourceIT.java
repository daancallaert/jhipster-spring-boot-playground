package be.daancallaert.web.rest;
...
import be.daancallaert.IntegrationTest;
import be.daancallaert.config.EmbeddedKafka;
import be.daancallaert.config.KafkaSseConsumer;
import be.daancallaert.config.KafkaSseProducer;
...
@WithMockUser
@EmbeddedKafka
class JhipsterplaygroundKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/jhipsterplayground-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/jhipsterplayground-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/jhipsterplayground-kafka/unregister"));
                return;
            }
        }
